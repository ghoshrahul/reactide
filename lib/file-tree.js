...
var projInfoPath = path.join(__dirname, "../lib/projInfo.json");
      ...
      files.forEach(file => {
        ...
        fs.stat(filePath, (err, stats) => {
          ...
          if (stats && stats.isFile()) {
            ...
            if (
              !projInfo.htmlPath &&
              filePath.search(/.*node\_modules.*/) === -1 &&
              file.search(/.*index\.html/) !== -1
            ) {
              projInfo.htmlPath = filePath;
            }

            //look for package.json and see if webpack is installed and react-hot-loader is installed
            else if (
              !projInfo.webpack &&
              file.search(/package.json/) !== -1 &&
              filePath === path.join(projInfo.rootPath, file)
            ) {
